...
function add_meta_boxes() {
	add_meta_box( 'anime-metabox', __( 'Anime Relationship', 'anva-anime-manager' ), 'anime_field', 'anime_episode' );
}
...
function anime_field() {
	...
	<table class="form-table">
		<tr valign="top">
			<th scope="row">
				<label for="anime"><?php _e( 'Anime', 'anva-anime-manager' ); ?></label>
			</th>
			<td>
				<select name="anime">
					<option value="">
						<?php _e( 'Select the anime', 'anva-anime-manager' ); ?>
					</option>
					<?php foreach ( $all_anime as $anime ) : ?>
						<option value="<?php echo $anime->ID; ?>" <?php echo selected( $selected_anime, $anime->ID, false); ?>>
